...
local function createReloadButton( f, placement, offX, offY )
    ...
    reloadButton:SetText( L["RELOAD_BUTTON"])
...
function display:createHelpFrame( title )
...
function display:createCombatEventLog( title )
...
function display:createErrorMsgFrame(title)
...
function display:createMsgFrame( title )
...
function display:clearFrame(f) -- erases the text in the edit box
	if f == nil then
		return
	end
	f.Text:EnableMouse( false )    
